...
const Login = () => {
    ...
    const handleSingInSignUp = () =>{
        const message = checkValidData(email.current.value,password.current.value, username?.current?.value, isSignInForm)
  ...
  return (
    <div>
        ...
        <form onSubmit={(e)=>e.preventDefault()} className='absolute w-3/12 p-12 bg-black bg-opacity-80 rounded-lg text-white my-40 mx-auto left-0 right-0'>
            ...
            <button className='p-4 my-6 w-full bg-red-700 rounded-lg' onClick={handleSingInSignUp}>{isSignInForm?"Sign In":"Sign Up"}</button>
